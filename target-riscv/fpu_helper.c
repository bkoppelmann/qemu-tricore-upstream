 ...
 * TODO better way...
 */
static unsigned int softfloat_flags_to_riscv(unsigned int flag)
{
    switch (flag) {
...
#define set_fp_exceptions() do { env->csr[NEW_CSR_FFLAGS] |= \
        softfloat_flags_to_riscv(get_float_exception_flags(&env->fp_status)); \
        set_float_exception_flags(0, &env->fp_status); } while (0)
...
static uint_fast16_t float32_classify(uint32_t a)
{
    union ui32_f32 uA;
    uint_fast32_t uiA;
    ...
    return
        ...
        (isNaNF32UI(uiA) && float32_is_signaling_nan(uiA, NULL)) << 8 |
        (isNaNF32UI(uiA) && !float32_is_signaling_nan(uiA, NULL)) << 9;
}
